...
def startup_details(startup_name):
    ...
    with col1:
        st.title('Industry')
        industry_data = df[df['Startup'].str.contains(startup_name, na=False)][
            'Vertical'].str.lower().value_counts().head()
    ...
    with col2:
        ...
        sub_industry_data = df[df['Startup'].str.contains(startup_name, na=False)][
            'subvertical'].str.lower().value_counts().head()
    ...
    st.title('City')
    city_data = startup_df.groupby('City  Location')['investors'].count().sort_values(ascending=False).head()
